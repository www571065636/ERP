...
function viewDetail(row) {
  ...
  dialogVisible.value = true
  if (!row.items) {
    http.get(`/purchase/orders/${row.id}/`).then(res => { form.value = res })
  }
}
...
async function approveOrder(row) {
  ...
  await http.post(`/purchase/orders/${row.id}/approve/`, { action: 'approve' })
...
async function openReceiptDialog(row) {
  const res = await http.get(`/purchase/orders/${row.id}/`)
  currentReceiptOrder.value = res
  receiptItems.value = (res.items || []).map(item => ({
    ...item,
