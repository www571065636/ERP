  ...
  <div class="page-container">
    <div class="page-header">
      <div class="page-header-left">
        ...
        <div>
          <div class="page-header-title">应付账款</div>
          <div class="page-header-sub">跟踪供应商应付款项与付款进度</div>
        </div>
      </div>
    </div>
    ...
    <div class="table-card">
      <div class="table-toolbar">
        <span class="table-toolbar-title">应付账款列表</span>
        <el-tag effect="light" type="info">共 {{ total }} 条</el-tag>
      </div>
      <el-table :data="list" v-loading="loading" stripe>
        <el-table-column prop="payable_no" label="应付单号" min-width="160">
          <template #default="{ row }">
            <el-tag effect="light" type="warning" size="small">{{ row.payable_no }}</el-tag>
          </template>
        </el-table-column>
        <el-table-column prop="supplier_id" label="供应商ID" width="80" align="center" />
        ...
        <el-table-column prop="paid_amount" label="已付金额" width="110" align="right">
          <template #default="{ row }">
            <span style="color:#52c41a">¥{{ row.paid_amount }}</span>
          </template>
        </el-table-column>
        <el-table-column prop="balance" label="未付余额" width="110" align="right">
          <template #default="{ row }">
        ...
        <el-table-column label="操作" width="140" fixed="right" align="center">
          <template #default="{ row }">
            <div class="action-btns">
              <el-button v-if="auth.hasPermission('finance:payable:payment')" text type="primary" size="small"
    ...
    <el-dialog v-model="paymentVisible" title="付款登记" width="400px" destroy-on-close>
      <el-form :model="payForm" label-width="100px" class="dialog-form">
        <el-form-item label="应付单号">
          <span style="color:#faad14;font-weight:500">{{ payForm.payable_no }}</span>
        </el-form-item>
      ...
      <template #footer>
        ...
        <el-button type="primary" :loading="saving" @click="handlePayment">确认付款</el-button>
      </template>
    </el-dialog>
  </div>
</template>
...
const statusLabels = { 0: '未付', 1: '部分付', 2: '全部付', 9: '已核销' }
...
async function loadData() {
  ...
  try {
    ...
    const res = await http.get('/finance/payables/', { params })
...
async function handlePayment() {
  saving.value = true
  try {
    await http.post(`/finance/payables/${payForm.value.id}/payments/`, payForm.value)
